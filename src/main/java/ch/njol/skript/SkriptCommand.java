		...
		if (args[0].equalsIgnoreCase("reload")) {
			...
			if (args[1].equals("all")) {
				try {
					...
					error(sender, "Could not enable any scripts (some scripts might have been renamed already and will be enabled when the server restarts): " + e.getLocalizedMessage());
				}
			} else {
					...
					switch (state) {
						...
						case CHECKED_FOR_UPDATE:
							if (Updater.latest == null)
								info(sender, "" + Updater.m_running_latest_version);
							else
